<?php
	...
	if($settings_details['envato_buyer_name']=='' OR $settings_details['envato_purchase_code']=='' OR $settings_details['envato_purchased_status']==0) {
		...
		$set['LIVETV'] =array('msg' => 'Purchase code verification failed!','success'=>'0');
	...
	function get_user_status($user_id)
	{
 	...
 	if($get_method['method_name']=="get_home")
  	...
  	else if($get_method['method_name']=="get_movies_by_gen_id")
  	{
		...
		else if($get_method['method_name']=="get_hightlight_by_lang_id")
		{
		...
		$query="SELECT movie.*` FROM tbl_highlights movie
				WHERE movie.`status`='1' AND   ORDER BY movie.`id` DESC LIMIT $limit, $page_limit";
		...
		else if($get_method['method_name']=="get_hightlight_by_gen_id")
		{
